...
**責務**: ランクアップ時に自動付与される報酬アイテムの値オブジェクト

**フィールド**:
- Rank: 対象ランク
- Items: RankRewardItem配列

### RankRewardItem

**責務**: ランクアップ報酬の1アイテムを表す

**フィールド**:
- Category: "core" / "skill" / "chain_effect"
- TypeID: アイテムTypeID

**ルール**:
1. ランクアップ時に自動的にインベントリに追加
2. 同ランク全敵撃破により次ランク解放時にのみ付与
3. ランクアップ報酬のスキルにはチェイン効果を付与しない
4. マスタデータで定義されていないランクでランクアップした場合、報酬なし
